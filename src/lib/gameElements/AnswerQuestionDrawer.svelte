<script lang="ts">
	...
	let { disabled, children, character, drawerOpen = $bindable(), question = $bindable()} = $props();
</script>

<div hidden use:shortcuts={{ keys: ['q'], type: 'callback', fn: () => {drawerOpen = true;}}}></div>
<Drawer.Root bind:open={drawerOpen}>
	<Drawer.Trigger {disabled} hidden>{@render children()}</Drawer.Trigger>
	<Drawer.Content >
