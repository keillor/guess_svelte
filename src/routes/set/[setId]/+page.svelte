...
<div class='p-4'>
    ...
    <div class='grid grid-cols-2 md:flex justify-center justify-items-center gap-5'>
		...
		<SetDeleteConfirmation setId={data.set?.docId}>
            ...
            <a href={resolve(`/create/${data.set.docId}/edit`)} class='bg-blue-600 hover:bg-blue-800 transition-all p-2 rounded-2xl text-xl w-40 outline-1 text-center cursor-default'>Edit</a>
        {/if}
    </div>
